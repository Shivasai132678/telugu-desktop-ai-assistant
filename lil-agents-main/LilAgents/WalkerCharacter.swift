...
class WalkerCharacter {
    ...
    private var speechBubbleExpiry: CFTimeInterval = 0
    private var showingSpeechBubble = false
    ...
    func updateThinkingBubble() {
        ...
        if showingSpeechBubble {
            if now >= speechBubbleExpiry {
                showingSpeechBubble = false
                hideBubble()
                return
            }
            if isIdleForPopover {
                speechBubbleExpiry += 1.0 / 60.0
                hideBubble()
            } else {
                showBubble(text: speechBubbleText, isCompletion: false)
            }
            return
        }
    ...
    func showSpeechBubble(text: String, duration: TimeInterval = 3.0) {
        ...
        speechBubbleText = trimmed
        showingSpeechBubble = true
        showingCompletion = false
        phraseAnimating = false
        speechBubbleExpiry = CACurrentMediaTime() + max(0.5, duration)
        if !isIdleForPopover {
            showBubble(text: speechBubbleText, isCompletion: false)
        }
    }
